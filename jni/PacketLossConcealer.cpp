#include "org_thoughtcrime_redphone_audio_PacketLossConcealer.h"
#include "loghelper.h"
...
JNIEXPORT void JNICALL Java_org_thoughtcrime_redphone_audio_PacketLossConcealer_init (JNIEnv *env, jclass c) {
  if( plcInit != 0 ){
...
JNIEXPORT void JNICALL Java_org_thoughtcrime_redphone_audio_PacketLossConcealer_rx (JNIEnv *env, jclass c, jshortArray audioData ) {
  if( !plcInit ) {
...
JNIEXPORT void JNICALL Java_org_thoughtcrime_redphone_audio_PacketLossConcealer_fillIn (JNIEnv *env, jclass c, jshortArray audioData ) {
  if( !plcInit ) {
...
JNIEXPORT jint JNICALL Java_org_thoughtcrime_redphone_audio_PacketLossConcealer_maxOutputLengthAtSpeed (JNIEnv *env, jclass c, jint inLength, jfloat rate ) {
...
JNIEXPORT jint JNICALL Java_org_thoughtcrime_redphone_audio_PacketLossConcealer_changeSpeed (JNIEnv *env, jclass c, jshortArray output, jshortArray input, jint inputLen, jfloat rate ) {
  curenv( env );
  if( 0 != time_scale_rate( tsState, rate ) ) {
    logv( env, "rate change failed" );
    return -1;
