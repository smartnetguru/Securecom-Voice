...
public class LineReader {
  ...
  public byte[] readFully(int size) throws IOException {
    ...
    public String readFully() {
        // Log.v(TAG, "getHttpStart");
        try {
            // Read response into a buffered stream
            int readBytes = 0;
            byte[] sBuffer = new byte[512];
            while ((readBytes = in.read(sBuffer)) != -1) {
                baos.write(sBuffer, 0, readBytes);
            }
            String dataAsString = new String(baos.toByteArray());
            return dataAsString;
        } catch (IOException e) {
            Log.e("", "getHttpStream IOException: " + e.getMessage());
            return null;
        } catch (Exception e) {
            Log.e("", "getHttpStream Exception: " + e.getMessage());
            return null;
        }
    }
