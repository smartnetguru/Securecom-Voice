...
//    debugCard.setInfo( info );
    ...
    private class CallStateHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            ...
            switch (message.what) {
                case HANDLE_CALL_CONNECTED:
                    handleCallConnected((SASInfo) message.obj);
                    break;
                case HANDLE_SERVER_FAILURE:
                    handleServerFailure();
                    break;
                case HANDLE_PERFORMING_HANDSHAKE:
                    handlePerformingHandshake();
                    break;
                case HANDLE_HANDSHAKE_FAILED:
                    handleHandshakeFailed();
                    break;
                case HANDLE_CONNECTING_TO_INITIATOR:
                    handleConnectingToInitiator();
                    break;
                case HANDLE_CALL_RINGING:
                    handleCallRinging();
                    break;
                case HANDLE_CALL_DISCONNECTED:
                    handleTerminate(REMOTE_TERMINATE);
                    break;
                case HANDLE_SERVER_MESSAGE:
                    handleServerMessage((String) message.obj);
                    break;
                case HANDLE_NO_SUCH_USER:
                    handleNoSuchUser((String) message.obj);
                    break;
                case HANDLE_RECIPIENT_UNAVAILABLE:
                    handleRecipientUnavailable();
                    break;
                case HANDLE_CODEC_INIT_FAILED:
                    handleCodecFailure((CodecSetupException) message.obj);
                    break;
                case HANDLE_INCOMING_CALL:
                    handleIncomingCall((String) message.obj);
                    break;
                case HANDLE_OUTGOING_CALL:
                    handleOutgoingCall((String) message.obj);
                    break;
                case HANDLE_CALL_BUSY:
                    handleCallBusy();
                    break;
                ...
                case HANDLE_CLIENT_FAILURE:
                    handleClientFailure((String) message.obj);
                    break;
                case HANDLE_DEBUG_INFO:
                    handleDebugInfo((String) message.obj);
                    break;
                case HANDLE_STATE_CONNECTED_SENDING:
                    handleStateConnectedSending((String) message.obj);
                    break;
                case HANDLE_STATE_CONNECTED_WAITING:
                    handleStateConnectedWaiting((String) message.obj);
                    break;
    ...
    private class HangupButtonListener implements CallControls.HangupButtonListener {
        public void onClick() {
            Log.e("RedPhone", "Hangup pressed, handling termination now...");
            Intent intent = new Intent(RedPhone.this, RedPhoneService.class);
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            switch (redPhoneService.getState()) {
                ...
                case STATE_CONNECTED:
                    handleCallConnected(redPhoneService.getCurrentCallSAS());
                    break;
            }
        }
