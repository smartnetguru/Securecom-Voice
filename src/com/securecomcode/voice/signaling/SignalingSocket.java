...
public class SignalingSocket {
    ...
    public SignalingSocket(Context context, String host, int port,
        ...
        try {
            ...
            this.socket = constructSSLSocket(context, signalingHost, signalingPort, false);
    ...
    private Socket constructSSLSocket(Context context, String host, int port, boolean othersignal)
            throws SignalingException {
        try {
            ...
            return timeoutHackConnect(sslSocketFactory, host, port, othersignal);
        } catch (IOException ioe) {
            throw new SignalingException(ioe);
    ...
    private Socket timeoutHackConnect(SSLSocketFactory sslSocketFactory, String host, int port, boolean othersignal)
            throws IOException {
    ...
    public SessionDescriptor initiateConnection(String remoteNumber)
        ...
        switch (response.getStatusCode()) {
            ...
            case 402:
                throw new ServerMessageException(new String(response.getBody()));
            case 401:
                throw new LoginFailedException("Initiate threw 401");
            case 200:
                return gson.fromJson(new String(response.getBody()), SessionDescriptor.class);
            default:
                throw new SignalingException("Unknown response: " + response.getStatusCode());
        }
    }
    ...
    public void setRinging(long sessionId)
        ...
        switch (response.getStatusCode()) {
            case 404:
                throw new SessionStaleException("No such session: " + sessionId);
            case 401:
    ...
    public void setHangup(long sessionId) {
        try {
            ...
            try {
                this.socket = constructSSLSocket(context, signalingHost, signalingPort, true);
            ...
            synchronized (SignalingSocket.this) {
                try {
                    if (!SignalingSocket.this.connectionAttemptComplete)
                        SignalingSocket.this.wait(10000);
